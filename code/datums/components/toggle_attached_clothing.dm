...
/datum/component/toggle_attached_clothing/proc/on_checked_overlays(obj/item/source, list/overlays, mutable_appearance/standing, isinhands, icon_file)
	...
	if (isinhands || currently_deployed)
		return
	// DOPPLER EDIT ADDITION START - Autogenerated clothing ignores undeployed sprites
	var/obj/item/parent_gear = parent
	if (ishuman(parent_gear.loc))
		var/mob/living/carbon/human/human_owner = parent_gear.loc
		if (!isnull(parent_gear.get_matching_bodyshape(human_owner)))
			return // sorry, i cant wrap my head around making autogenned undeployed sprites rn...
